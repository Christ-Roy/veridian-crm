...
| `veridian-record-open/utils/recordOpenManager.ts` | `getDefaultStore` de `jotai` (écrit le pending atom HORS React — Twenty n'a pas de `<Provider store>` custom, donc même store que les rows) ; `veridianPendingOpenKeysState` + `VERIDIAN_RECORD_OPEN_DELAY_MS` (modules Veridian). MANAGER module-level : Map de timers (décompte qui SURVIT au démontage), pending atom (scintillement row), garde d'idempotence (1 écriture/ouverture). Expose `scheduleRecordOpen` / `cancelRecordOpen` / `isRecordOpenPending` / `buildRecordOpenKey`. | 2026-06-17 |
