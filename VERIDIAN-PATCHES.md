...
| `database/commands/upgrade-version-command/2-13/2-13-workspace-command-1781277460000-sync-standard-ui-capability-flags.command.ts` | lecture du drift en **SQL brut** (`coreDataSource.query`) au lieu de `getOrRecompute` ORM + `flush` (au lieu de `invalidateAndRecompute`) → évite le `SELECT isUIReadOnly` fantôme qui deadlock le cross-version upgrade (cf `todo/2026-06-14-upgrade-ui-capability-flags-fail.md`). Fix #21543 upstream a corrigé l'écriture mais laissé la lecture sur l'ORM. | `2-13/__tests__/sync-standard-ui-capability-flags.veridian.spec.ts` (lecture SQL brute, jamais `getOrRecompute` flat-metadata, `flush` pas `invalidateAndRecompute`) |
