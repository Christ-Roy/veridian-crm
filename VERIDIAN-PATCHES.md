...
| `twenty-front/src/modules/views/components/ViewBar.tsx` | monte `<VeridianProspectionFilterButtons />` dans le `rightComponent` du `TopBar` (entre `<ObjectSortDropdownButton />` et `{optionsDropdownButton}`) → cockpit prospection : dropdown **Taille** (Individuel 0-2 / PME 3-249 / Grande 250+ sur le champ `effectifs`) + boutons rapides **Avec/Sans site** (`hasWebsite` true/false). **ViewBar est PARTAGÉ par tous les objets → le gating sur `company` (objectNameSingular) est fait DANS le module**, pas dans le core ; rien ne s'affiche ailleurs. Applique les filtres via le vrai système (`useUpsertRecordFilter`/`useRemoveRecordFilter` sur `currentRecordFiltersComponentState` keyé `recordIndexId`) → la table refetch seule, aucun store maison. Ids de filtre STABLES (toggle on/off, pas d'empilement). Logique 100% dans `modules/veridian-prospection-filters/` ; patch upstream = 1 import + 1 balise JSX. | `veridianProspectionFilter.veridian.spec.ts` (gating company-only, presets, résolution du preset/site actif) |
