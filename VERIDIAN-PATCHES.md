...
| `twenty-front/src/modules/object-record/record-show/components/PageLayoutRecordPageRenderer.tsx` | monte `<VeridianRecordOpenEffect objectNameSingular recordId>` (dérivé de `targetRecordIdentifier`) dans `StyledShowPageRightContainer` → mécanique "ouverture de fiche" (timer 5s → horodate `ficheOuverteAt` + `ficheOuverteParId` + progression `statutColdCall` A_APPELER→FICHE_OUVERTE only, jamais de régression ; cf VISION-INSTANCE-TWENTY-CUSTOM §4). **Monté ICI (et non dans `RecordShowPage`) car ce renderer est rendu DANS LES DEUX contextes record — pleine page (`isInSidePanel=false`) ET side-panel (`isInSidePanel=true`) → un seul point de montage couvre les deux.** 2e patch sur ce fichier : `position: relative` ajouté sur `StyledShowPageRightContainer` (containing-block de l'overlay absolu de l'indicateur ; sans offset → aucun impact layout). Le dé-doublonnage des écritures entre les 2 instances simultanées (side-panel + pleine page de la même fiche) est géré par la garde module-level `recordOpenGuard`. Logique 100% dans le module neuf `veridian-record-open` ; patch upstream = 1 import + 1 balise JSX + 1 propriété CSS. | `PageLayoutRecordPageRenderer.veridian.spec.tsx` (l'effet est monté dans les 2 valeurs de `isInSidePanel` + reçoit objectNameSingular/recordId) |
| `twenty-front/src/modules/object-record/record-table/record-table-row/components/RecordTableTr.tsx` | lit l'atom global `veridianActiveOpenKeyState` et pose `data-veridian-record-opening` sur la row quand l'openKey actif == `<objectNameSingular>:<recordId>` de cette row → anime la LIGNE dans la vue table pendant la fenêtre d'annulation 5s (point (c) de la mécanique d'ouverture). `RecordTableTr` est le chokepoint unique des rows de données visibles (Static/Draggable/FirstRowOfGroup délèguent toutes ici). Patch = 2 imports + 1 hook lecture + 1 `data-*`. | `RecordTableTr.veridian.spec.tsx` (data-attr posé ssi openKey actif == row, sinon non) |
| `twenty-front/src/modules/object-record/record-table/record-table-row/components/RecordTableRowDiv.tsx` | bloc CSS `&[data-veridian-record-opening='true']` + `@keyframes veridian-row-open-pulse` (glow bleu pulsant sur les cellules) → rendu visuel de l'animation de row (point (c)). Pure CSS additive, n'altère pas les états `focused`/`active` natifs. **Cosmétique — non testé en runtime** (Linaria n'extrait pas la CSS en jsdom ; le wiring qui PILOTE l'attribut est couvert par `RecordTableTr.veridian.spec.tsx`). Un sync qui réécrirait ce styled-component effacerait le glow silencieusement → à re-vérifier visuellement au sync. | cosmétique — non testé (wiring couvert côté `RecordTableTr`) |
...
| `veridian-record-open/components/VeridianRecordOpenEffect.tsx` | `useUpdateOneRecord` de `@/object-record/hooks` ; `currentWorkspaceMemberState` de `@/auth/states` ; `useAtomStateValue` de `@/ui/utilities/state/jotai/hooks` ; `useStore` de `jotai` + `recordStoreFamilySelector` de `@/object-record/record-store/states/selectors` (lecture one-shot du `statutColdCall` courant) ; `isDefined` de `twenty-shared/utils`. Tout rename de ces chemins par un sync casse le typecheck/build front (révélé par CI + build Docker). | 2026-06-17 |
| `veridian-record-open/components/VeridianRecordOpenIndicator.tsx` | `styled` de `@linaria/react` + `themeCssVariables` de `twenty-ui-deprecated/theme-constants` (tokens `color.blue`, `background.transparent.blue`). Overlay d'animation de la fenêtre d'annulation 5s (glow pulsant + barre de progression), pattern `@keyframes` inline cf `ai/components/ShimmeringText.tsx`. Durée pilotée par CSS var `--veridian-open-duration` = `VERIDIAN_RECORD_OPEN_DELAY_MS`. | 2026-06-17 |
| `veridian-record-open/utils/recordOpenGuard.ts` | aucune dépendance upstream (état module-level pur). Garde d'idempotence PARTAGÉE entre instances de l'Effect → dé-doublonnage des écritures side-panel ⟷ pleine page. | 2026-06-17 |
| `veridian-record-open/states/veridianActiveOpenKeyState.ts` | `createAtomState` de `@/ui/utilities/state/jotai/utils/createAtomState`. Atom global réactif portant l'openKey de la fenêtre active → consommé par `RecordTableTr` (point (c)) et l'Effect (set/clear). | 2026-06-17 |
